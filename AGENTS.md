...
The Phase 0 branch contains a reviewable read-only capability harness and a deliberately narrow Phase 0 Worker under src/, scripts/, migrations/, and test/. Treat it as a connectivity artifact, not as the application UI or permission to add seller operations. The Worker routes are limited to `/phase0/oauth/start`, `/phase0/oauth/callback`, and `/phase0/capability`; do not add product UI, seller writes, or Phase 1 features without explicit instruction.
